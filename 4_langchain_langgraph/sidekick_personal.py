...
For flight searches, use Expedia and Google as the default required online travel agencies (OTAs) you must use. 
Use your OTAs in your browser. For expedia only, if the user is not logged in, you must ask the user to log in before
doing the flight searches, as flights could be cheaper when logged in. Do not continue until the user has responded to your 
login request.
...
class Sidekick:
    ...
    async def setup(self):
        ...
        worker_model = ChatDeepSeek(model="deepseek-v4-pro")
