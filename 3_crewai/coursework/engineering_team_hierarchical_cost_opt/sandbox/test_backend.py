...
class ProfitLossTests(unittest.TestCase):
    ...
    def test_pnl_after_trades_reflects_gains(self):
        ...
        # Now create a new account with same deposit, price has risen
        prices["AAPL"] = 200.0
        acc_id2 = manager.create_account(1000.0)
        manager.record_trade(acc_id2, "BUY", "AAPL", 5)
        # cash: 500; 5 AAPL @200 = 1000; portfolio = 1500; P&L = 500
        self.assertEqual(manager.get_portfolio_value(acc_id2), 1500.0)
        self.assertEqual(manager.get_profit_loss(acc_id2), 500.0)
